...
export const DataProvider = ({ children }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [transRes, goalsRes, debtsRes, investRes] = await Promise.all([
                ...
                axios.get('http://localhost:5000/api/debts', { headers: { 'x-auth-token': token } }),
                axios.get('http://localhost:5000/api/investments', { headers: { 'x-auth-token': token } })
            ]);
    ...
    const updateGoal = async (id, currentAmount) => {
        const res = await axios.put(`http://localhost:5000/api/goals/${id}`, { currentAmount });
        setGoals(goals.map(g => g._id === id ? res.data : g));
    };

    const addDebt = async (debt) => {
        const res = await axios.post('http://localhost:5000/api/debts', debt);
        setDebts([res.data, ...debts]);
    };

    const updateDebt = async (id, data) => {
        const res = await axios.put(`http://localhost:5000/api/debts/${id}`, data);
    ...
    const updateInvestment = async (id, data) => {
        const res = await axios.put(`http://localhost:5000/api/investments/${id}`, data);
        setInvestments(investments.map(inv => inv._id === id ? res.data : inv));
    };

    const refreshInvestments = async () => {
        const res = await axios.get('http://localhost:5000/api/investments', { headers: { 'x-auth-token': token } });
